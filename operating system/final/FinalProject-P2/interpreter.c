...
int readCommandEXAM(char *parsedCommand[]){
    ...
    // Read the string from the file
    resetCurrentLocation(FAT_index);
    
    char *ptr = readBlock(FAT_index);
    if(ptr ==  NULL){
        printf("Read command failed.\n");
        return 2; // Read block failed
    }
...
int closeCommandEXAM(char *parsedCommand[]){
    ...
    int index = atoi(parsedCommand[1]);         // Translate string to int. Index of active file table.
