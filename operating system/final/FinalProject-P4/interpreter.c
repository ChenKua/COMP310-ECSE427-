...
int readpipeCommadnEXAM(char *parsedCommand[]);
...
int interpreter(char *parsedCommand[]){
    ...
    // New commands for final project TODO
        ...
        return readpipeCommadnEXAM(parsedCommand);
...
int printCommand(char *parsedCommand[]){
    ...
    if (value != NULL && strlen(value) != 0)                    // modified
        printf("%s\n",value);
    else
...
int writeCommandEXAM(char *parsedCommand[]){
    ...
    int index = atoi(parsedCommand[1]);  //translate string to int

    // COPY FROM write command
    // Open file only opens files once, so there is no need to check if it is open already
    // int fileID = openFile(parsedCommand[1]);
    
    // check if file is opened
    // Error Code: 7
    int err = findFileInActiveTable(index);
    if (err == -1) {
        printf("ERROR: Open the file first.\n");
        return 7;
    }

    int FAT_index   = active_file_table_owners[index];     // file ID in fat[20]
...
int readCommandEXAM(char *parsedCommand[]){
    ...
    // Read the string from the file
    // resetCurrentLocation(index);

    char *ptr = read_EXAM(FAT_index);
    if(ptr ==  NULL){
        printf("Read command failed.\n");
        return 2; // Read block failed
    }
...
int openCommandEXAM(char *parsedCommand[]){
    ...
    int err = openFile_EXAM(parsedCommand[1], parsedCommand[2]);
    return err;
}
...
int closeCommandEXAM(char *parsedCommand[]){
    ...
    int index = atoi(parsedCommand[1]);         // Translate string to int
    int err   = findFileInActiveTable(index);   // Check if this index is in use
    if (err == -1) {
        printf("ERROR: Index number not in use.\n");
        return 7;       // Honestly it does not matter what I retrun here becuase we dont have a errno.c 
                        // for this project. But I will just keep this convertion.
    }
    close_EXAM(index);
    clearBuffer(index);
    return 0;
...
int readpipeCommadnEXAM(char *parsedCommand[]){
    ...
    // If it is normal mode, call read_EXAM
    ...
    if (strcmp(parsedCommand[2], "|") != 0) {
        int err = readCommandEXAM(parsedCommand);
        return err;
    } 

    // Argument Count Error handeling
    if (parsedCommand[3] == NULL || parsedCommand[4] == NULL ){
        printf("Wrong syntax. The read command requires a filename and a shell variable name as arguments.\n");
        return 1;
    }

    // Check if the index is correct
    int index = atoi(parsedCommand[1]);         // Translate string to int. Index for active file table.
    int err   = findFileInActiveTable(index);   // Check if this index is in use
    if (err == -1) {
        printf("ERROR: Index number not in use.\n");
        return 7;       // Honestly it does not matter what I retrun here becuase we dont have a errno.c 
    ...
    strcpy(pipeBuffers[FAT_index], ptr);

    while(ptr != NULL){
        ptr = read_EXAM(FAT_index);
        if (ptr == NULL)
            break;
        strcat(pipeBuffers[index], ptr);
    }

    char tmp[1000];
    strcpy(tmp, pipeBuffers[index]);
    // Write the string to the shell variable
    setVariable(parsedCommand[4], tmp);
    // closeFile(index);
